...
@RestController
@RequestMapping("/api/my-learning-path-kafka")
public class MyLearningPathKafkaResource {

    private final Logger log = LoggerFactory.getLogger(MyLearningPathKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public MyLearningPathKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
