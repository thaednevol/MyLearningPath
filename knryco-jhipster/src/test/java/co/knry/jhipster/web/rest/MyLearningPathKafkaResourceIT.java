...
import co.knry.jhipster.config.EmbeddedKafka;
import co.knry.jhipster.config.KafkaSseConsumer;
import co.knry.jhipster.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class MyLearningPathKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/my-learning-path-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/my-learning-path-kafka/consume")
